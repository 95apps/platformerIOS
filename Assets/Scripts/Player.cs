...
public class Player : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        if(platforms.consecutiveJumped < 4){
            rb.velocity = new Vector3(Input.acceleration.x, rb.velocity.y, -Input.acceleration.z);
        } else if(platforms.consecutiveJumped >= 4 && platforms.consecutiveJumped <8){
            rb.velocity = new Vector3(Input.acceleration.z, rb.velocity.y, -Input.acceleration.x);
    ...
    private void Jump()
    {
        ...
        if (rb.velocity.y == 0)
        {
			...
			if (Input.touchCount == 1 && Raycast.onGround == true )
            {
